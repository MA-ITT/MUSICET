    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            await CallbackQuery.message.reply(
                f"🎧 وقفتها موقتآ لحد متخلص {rpk}!", reply_markup=play_keyboard
            )
            await CallbackQuery.message.delete()
        else:
            await CallbackQuery.answer(f"مفيش حاجه شغالها!", show_alert=True)
            return
    else:
        await CallbackQuery.answer(f"مفيش حاجه شغالها يستا!", show_alert=True)
...
""",
    ...
    if await is_active_chat(chat_id):
        if await is_music_playing(chat_id):
            ...
            rpk = "[" + user_name + "](tg://user?id=" + str(user_id) + ")"
            await CallbackQuery.message.reply(
                f"🎧 شغلتلك الاغنيه يا{rpk}!", reply_markup=play_keyboard
            )
            await CallbackQuery.message.delete()
    else:
...
async def stopvc(_, CallbackQuery):
    ...
    if await is_active_chat(chat_id):
        ...
        user_name = CallbackQuery.from_user.first_name
        rpk = "[" + user_name + "](tg://user?id=" + str(user_id) + ")"
        await CallbackQuery.message.reply(f"🎧 تم ايقاف الاغنيه يا {rpk}!")
    else:
        await CallbackQuery.answer(f"لا يوجد شئ قيد التشغيل!", show_alert=True)
...
async def rabic(_, query: CallbackQuery):
    await query.edit_message_text(
        f"""
**[🥀]({BOT_IMG}) مساء النعنع والليمون☘️🍋**

**🧸 انا انا بـوت بـشـغـل اغـانـي فـي الدردشة الصوتية ويمكنه تشغيل مقاطع الفيديو في الدردشة الصوتية!**

**🌹 تقدر تحديد اللغة من الازرار اللي تحت**
...
""",
        ...
        [
            InlineKeyboardButton("جروب السورس💖", url=f"https://t.me/{GROUP}"),
            InlineKeyboardButton("قناة السورس📣", url=f"https://t.me/{CHANNEL}"),
        ],
        [
            InlineKeyboardButton("الاوامر 🎸", url="https://t.me/WD_RG/5"),
        ],
        [
           InlineKeyboardButton("مطور السورس🥀", callback_data="vhkcbt"),
        ],
        [
...
async def vhkcbt(_, query: CallbackQuery):
    await query.edit_message_text(
        f"""[♥╎]({BOT_IMG}) أهـلا بيـك يـ بـرو فـي  قـسـم مطور السورس يمـكنـك الـتواصـل مـع المـطـور مـن خـلال الضـغـط بالأسـفـل""",
        reply_markup=InlineKeyboardMarkup(
            [
                [        
                    InlineKeyboardButton(
