...
@Client.on_message(command(["/play", "ايطالي" ,"مارلين" ,"شغل" ,"تشغيل", f"/play@{BOT_USERNAME}", "تشغيل"]))
async def play(_, message: Message):
    chat_id = message.chat.id
    if message.sender_chat:
        return await message.reply_text(
            """
معلش ياصحبي بس نت مش واخد رول اصلا كلم صاحب البار قلو.
"""
        )
...
"""
    ...
    if not a.can_delete_messages:
        await message.reply_text(
            "ياعم ادي البوت الصلاحيات كامله عشان يشتغل"
            + "\n❌ حذف الرسايل"
        )
        return
    ...
    try:
        ...
        if b.status == "kicked":
            await message.reply_text(
                f"""
{ASSNAME}(@{ASSUSERNAME}) نت يستا حظرت الاك **{chat_title}**

الغي الحظر الاكوانت المساعد عشانتعرف تستخدم البوت ياصحبي🎻
"""
            )
            return
        ...
        if message.chat.username:
            try:
                ...
                await message.reply(
                    f"{ASSNAME} تم أنضمام الحساب المسأعد بنجآح ي زميكس 🎧 ،،",
                )
...
"""
            ...
            try:
                ...
                await message.reply(f"{ASSNAME} تم أنضمام المساعد بنجآح ي زميكس . ✅",)
                await remove_active_chat(chat_id)
...
"""
    ...
    if audio:
        ...
        mystic = await message.reply_text(
            f"*جـاري معالجه الصوت عشان عيان️ {username}**"
        )
...
**مهو نا مش هسيبك تحرقلي السيرفر: {DURATION_LIMIT}**
**ده الوقت المقبول:** {duration}
"""
            )
        file_name = (
...
`تشغيل او شغل او ايطالي او مارلين + أسم المقطع الصوتي 🎧`
""",
            )
            return
        what = "Query Given"
        await LOG_CHAT(message, what)
        query = message.text.split(None, 1)[1]
        mystic = await message.reply_text("****اصبر بدور علي الاغنيه اهو... 🔍****")
        try:
...
"""
    ...
    if await is_active_chat(chat_id):
        ...
        m = await CallbackQuery.message.reply_photo(
            photo=thumb,
            caption=f"""
<b>🎧 وزودنا اغنيه في الانتظار اهي</b>

<b>🖋 الاسم Kontol:</b>[{title[:25]}]({url})
<b>🕓 الوقت Kontol:</b> {duration}
<b>🎧</b> [My Bot](https://t.me/{BOT_USERNAME}?start=info_{id})
<b>🎧 اللي طلبها:</b> {checking}

<b>#️⃣ الاغنيه دي رقم</b> {position}
""",
            reply_markup=InlineKeyboardMarkup(buttons),
        )
        os.remove(thumb)
